...
api=json.loads(urllib.request.urlopen("http://freegeoip.net/json/").read().decode("UTF-8"))
...
def getsunrise(lat="", lng="", formatted=1):
 ...
 print (url)
 sunapi=urllib.request.urlopen(url)
...
def getsunset(lat="", lng="", formatted="1"):
 if lat=="" or lng == "": 
  lat=latitude
  lng=longitude
 sunapi=urllib.request.urlopen("http://api.sunrise-sunset.org/json?lat=" + lat + "&lng=" + lng + "&formatted=" + str(formatted))
